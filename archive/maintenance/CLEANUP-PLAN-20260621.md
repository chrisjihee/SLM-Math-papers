...
> **HISTORICAL — 1차 archive split 계획서.** 이미 실행 완료되어 `archive/maintenance/`에 보존된 기록이다.
> 본문의 경로(`contents/`, `paper/01.` 등)는 1차 이동 당시 기준이며 현재 구조와 다를 수 있다(2차 simplification에서 다수 삭제됨).
> 현재 상태는 `../../REPO-CORPUS-STATUS-20260621.md`와 `../README.md`를 본다.
